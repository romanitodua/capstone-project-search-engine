...
func TestVocal(t *testing.T) {
	...
	defer inFile.Close() // Ensure inFile is closed after the function finishes
	in := bufio.NewReader(inFile)

	outFile, err := os.Open("out.txt")
	if err != nil {
		panic(err)
	}
	defer outFile.Close() // Ensure outFile is closed after the function finishes
	...
	for word, err := in.ReadSlice('\n'); err == nil; word, err = in.ReadSlice('\n') {
		...
		fmt.Println(string(Stem(word))) // Assuming Stem is defined elsewhere
